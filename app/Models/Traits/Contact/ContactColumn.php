...
trait ContactColumn
{
    ...
    public static function create_columns()
    {
        return collect(Schema::getColumnListing(Contact::getQuery()->from))
            ->reject(function ($column) {
                return in_array($column, ['id', 'slug', 'user_id', 'status', 'description', 'created_at', 'updated_at', 'deleted_at']);
            })
            ->toArray();
    }
    public static function edit_columns()
    {
        return collect(Schema::getColumnListing(Contact::getQuery()->from))
            ->reject(function ($column) {
                return in_array($column, ['id', 'status', 'slug', 'user_id', 'description', 'created_at', 'updated_at', 'deleted_at']);
            })
            ->toArray();
