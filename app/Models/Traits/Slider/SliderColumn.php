...
trait SliderColumn
{
    ...
    public static function create_columns()
    {
        return collect(Schema::getColumnListing(Slider::getQuery()->from))
            ->reject(function ($column) {
                return in_array($column, ['id', 'slug', 'thumbnail', 'status', 'deleted_at', 'created_at', 'updated_at', 'deleted_at']);
            })
            ->toArray();
    }
    public static function edit_columns()
    {
        return collect(Schema::getColumnListing(Slider::getQuery()->from))
            ->reject(function ($column) {
                return in_array($column, ['id', 'status', 'slug', 'thumbnail', 'created_at', 'updated_at', 'deleted_at']);
            })
            ->toArray();
