...
<div class="mt-2">
    <div class="edit_model_heading">
        <h5 class="text-center">Edit {{auth()->user()->name}}'s Profile</h5>
    </div>
    <br>
    <div class="view_button">
        <a href="{{url('admin/'.$model)}}" class="btn btn-primary btn-sm"><i class="far fa-eye"></i> View {{auth()->user()->name}}'s' Profile</a>
    </div>

    <form action="{{url('admin/userprofile/'.$data->slug)}}" method="post" enctype="multipart/form-data">
        @csrf
        {{method_field('PATCH')}}
        <div class="row">
