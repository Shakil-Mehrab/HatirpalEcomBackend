<!-- unit  -->
<div class="form-group {{ $errors->has('unit') ? ' has-error' : '' }} col-lg-4 col-md-4 col-sm-12 my-3"
    style="order:3">
...
<!-- waranty  -->
<div class="form-group {{ $errors->has('waranty') ? ' has-error' : '' }} col-lg-4 col-md-4 col-sm-12 my-3"
    style="order: 2;">
    @php
        $waranty = '';
    ...
    <input type="text" class="form-control" name="waranty" id="waranty"
        placeholder="01days 01 weeeks 01 months 01 years" value="{{ old('waranty') ? old('waranty') : $waranty }}">
...
<div style="order: 9;">
    ...
    <div class="toggle_div_des hidden">
        <div
            class="form-group{{ $errors->has('short_description') ? ' has-error' : '' }} col-lg-12 col-md-12 col-sm-12">
            <label for="short_description" class="control-label">Short Description</label>
            <textarea type="text" class="form-control my-editor" name="short_description" id="short_description"
                placeholder="Short Description" cols="30" rows="15">
        ...
        <div
            class="form-group{{ $errors->has('description') ? ' has-error' : '' }} col-lg-12 col-md-12 col-sm-12 mb-5">
            <label for="description" class="control-label">Description</label>
            <textarea type="text" class="form-control my-editor" name="description" id="description"
                placeholder="Description" cols="30" rows="15">
            @if (old('description'))
            {{ old('description') }}
        @else
...
<!-- related images  -->
<div class="form-group {{ $errors->has('images') ? ' has-error' : '' }} col-lg-4 col-md-4 col-sm-12 my-3"
    style="order: 12;">
    ...
    <div>
        @if (!empty($data->productImages))
            @forelse($data->productImages as $img)
                <img src="{{ file_exists($img->thumbnail) ? asset($img->thumbnail) : 'https://ui-avatars.com/api/?name=Hatirpal' }}"
                    alt="{{ asset($img->thumbnail) }}" width="50px" class="mt-2">
            @empty
            @endforelse
        @endif
    </div>
...
<!-- quantity  -->
<div class="form-group {{ $errors->has('stock') ? ' has-error' : '' }} col-lg-4 col-md-4 col-sm-12 my-3"
    style="order: 2;">
    @php
        $stock = '';
    ...
    <input type="text" class="form-control" name="stock" id="stock" placeholder="Stock"
        value="{{ old('stock') ? old('stock') : $stock }}">
    @if ($errors->has('stock'))
        <span class="help-block">
...
<!-- size  -->
<div class="form-group {{ $errors->has('size_id') ? ' has-error' : '' }} col-lg-4 col-md-4 col-sm-12 my-3"
    style="order: 3;">
    <label for="size_id" class="control-label">Select Size</label>
    <div>
        @forelse($sizes as $index=>$size)
            <input type="checkbox" class="form-checkbox" name="size_id[{{ $size->id }}]" id="size_id"
                value="{{ $size->id }}" @if (old('cat')) {{ old('size_id.' . $size->id) ? 'checked' : '' }} @else {{ $data ? ($data->sizes->contains($size->id) ? 'checked' : '') : '' }} @endif>
...
<!-- category  -->
<div class="form-group {{ $errors->has('category_id') ? ' has-error' : '' }} col-lg-12 col-md-12 col-sm-12 my-5"
    style="order: 4;">
    ...
    <div class="product_cat_div toggle_div_cat hidden">
        ...
        @forelse($categories as $category)
            <input type="checkbox" class="form-checkbox" name="category_id[{{ $category->id }}]" id="category_id"
                value="{{ $category->id }}" @if (old('cat')) {{ old('category_id.' . $category->id) ? 'checked' : '' }} @else {{ $data ? ($data->categories->contains($category->id) ? 'checked' : '') : '' }} @endif>
            <span>{{ $category->name }}</span>
            <div class="cat_child_div">
                @forelse($category->children as $cat)
                    <input type="checkbox" class="form-checkbox" name="category_id[{{ $cat->id }}]"
                        id="category_id" value="{{ $cat->id }}" @if (old('cat')) {{ old('category_id.' . $cat->id) ? 'checked' : '' }} @else {{ $data ? ($data->categories->contains($cat->id) ? 'checked' : '') : '' }} @endif>
                    <span>{{ $cat->name }}</span>
                    <br>
                    <div class="cat_child_div">
